...
def json_scrape_hjres_cosponsors(cong, num, n,x):
    response = requests.get(f'https://api.congress.gov/v3/bill/{cong}/hjres/{num}/cosponsors?limit=250&api_key={api_key[x]}')
    writeFile =open(os.path.join('/Users/Hugo/Desktop/Research/Research/Cosponsor', f'{cong}_hjres_{num}_cosponsors.json'), 'w')
    writeFile.write(response.text)
    writeFile.close()
    n=1
    if os.path.isfile(f'/Users/Hugo/Desktop/Research/Research/Cosponsor/{cong}_hjres_{num}_cosponsors.json') == True:
        print("complete")
        n+=1
        if n==975:
                time.sleep(3660)
...
def json_scrape_sjres_cosponsors(cong, num, n,x):
    ...
    writeFile =open(os.path.join('/Users/Hugo/Desktop/Research/Research/Cosponsor', f'{cong}_sjres_{num}_cosponsors.json'), 'w')
    writeFile.write(response.text)
    writeFile.close()
    if os.path.isfile(f'/Users/Hugo/Desktop/Research/Research/Cosponsor/{cong}_sjres_{num}_cosponsors.json') == True:
        print("complete")
        n+=1
        if n==975:
            time.sleep(3660)
        ...
        os.remove(os.path.isfile(f'/Users/Hugo/Desktop/Research/Research/Cosponsor/{cong}_sjres_{num}_cosponsors.json'))
        print("finished")
        return
...
#json_scrape_sjres_cosponsors(111, 1, 1, 14)
